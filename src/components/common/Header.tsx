...
import {Menu} from "@/components/common/mobile-menu/MobileMenu";
...
const CartIcon = ({dark}: { dark: boolean }) => (
    <svg
        width={40}
        height={41}
        viewBox="0 0 40 41"
    ...
    >
        <circle cx={20} cy="20.002" r="19.5" stroke={`${!dark ? "white" : "black"}`}/>
        <path
...
const SearchIcon = ({dark}: { dark: boolean }) => (
    ...
    >
        <circle cx={20} cy="20.002" r="19.5" stroke={`${!dark ? "white" : "black"}`}/>
        <circle cx="21.25" cy="17.752" r="6.25" stroke={`${!dark ? "white" : "black"}`}/>
        <path d="M17.5 23.0023L13 29.0023" stroke={`${!dark ? "white" : "black"}`} strokeLinecap="round"/>
    </svg>

)

const FavoriteIcon = ({dark}: { dark: boolean }) => (
...
export const Header = ({links, dark}: { links: IHeaderLink[], dark: boolean }) => {
    return (
        <div className="fixed w-full z-50 ">
            <header className="bg-transparent -mb-[100%] relative py-10">
                <div className="container ">
                    <div className="flex justify-between items-center">
                        <Link href="/" className="block select-none h-full w-auto">
                            <Image src={`/sk8-logo${dark ? "-black" : ""}.svg`}
                                   alt="sk8ology logo"
                                   width={220}
                                   height={50}/>
                        </Link>
                                ...
                                <ul className="flex">
                                    {links.map((link, index) => (
                                        <li key={link.url}
                                            id={`HeaderMenu-${index}`}>
                                            <Link href={link.url}
                                                  className={`mx-3 md:mx-4 lg:mx-5 xl:mx-6 text-sm md:text-base lg:text-lg ${!dark ? "text-white" : ""} font-semibold`}>{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </nav>
                        ...
                        <div className="gap-1 xl:gap-2.5 items-center hidden md:flex">
                            {[CartIcon, SearchIcon, FavoriteIcon].map((Icon, index) => (
                                <Link href="#"
                                      key={index}
                                      id={`HeaderIcon-${index}`}
                                      className={`h-10 w-10 list-none cursor-pointer ${!dark ? "text-white" : ""} rounded-full   header__icon header__icon--cart link focus-inset`}>
                                    <Icon dark={dark}/>
                                </Link>
                            ))}
                        </div>
